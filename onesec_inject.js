(function () {
    try {
        ...
        chrome.storage.local.get(['frictionDelay', 'oneSec_style'], async (res) => {
            let duration = (res.frictionDelay || 1) * 1000; // default 1s
            let style = res.oneSec_style || 'modal'; // default modal style
            ...
            overlay.style.fontFamily = 'Inter, Roboto, Arial, sans-serif';

            // apply style
            if (style === 'full-blur') {
                overlay.style.background = 'rgba(0,0,0,0.8)';
                overlay.style.backdropFilter = 'blur(8px)';
            } else if (style === 'progress-bar') {
            ...
            msg.textContent = `You are opening ${domain || 'this site'}. Take ${res.frictionDelay || 1} second${(res.frictionDelay || 1) > 1 ? 's' : ''} to decide.`;
